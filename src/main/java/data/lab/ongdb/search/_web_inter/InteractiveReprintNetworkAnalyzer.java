...
import data.lab.zdr.graph.common.relationship.Relationships;
import data.lab.zdr.graph.services.graph.virtual.VirtualGraphManager;
import data.lab.zdr.graph.services.visualinter.Neo4jDataUtils;
 ...
 * @Description: TODO(Reprint network analyzer)
 * @date 2020/3/3 15:40
 */
public class InteractiveReprintNetworkAnalyzer {

    // 互动类关系 (任意一条路径中关系列表中某一个关系不包含在此列表中则返回FALSE)
    public final static String relationships = "['" + Relationships.隶属虚拟账号 + "','" + Relationships.发帖 + "'," +
            "'" + Relationships.评论 + "','" + Relationships.转发 + "','" + Relationships.回复 + "','转评']";

    // 人物类标签
    public final static String lables = "['" + Labels.虚拟账号ID + "','" + Labels.新浪微博ID + "','" + Labels.网易博客ID + "'," +
            "'" + Labels.新浪博客ID + "','" + Labels.腾讯微博ID + "','" + Labels.TwitterID + "','" + Labels.百度贴吧ID + "'," +
            "'" + Labels.天涯论坛ID + "','" + Labels.微信公众号ID + "','" + Labels.FacebookID + "','" + Labels.LinkedinID + "'," +
            "'" + Labels.YouTubeID + "','" + Labels.InstagramID + "']";
...
//    private final static String relationshipsFieldName = "relationships";
    ...
    // 生成的虚拟关系名称
    private final static String virtualRelaName = "转评";
...
//    private final static CopyOnWriteArrayList<VirtualRelaId> virtualRelaIdManager = VirtualGraphManager.virtualRelaIdManager;
    ...
    public JSONObject analyzer(JSONObject reslut, long targetId) {
        ...
        // 过滤节点列表
        String[] labelSupports = new String[]{Labels.现实人员.toString(), Labels.TwitterID.toString(),
                Labels.FacebookID.toString(), Labels.YouTubeID.toString(), Labels.InstagramID.toString(), Labels.新浪微博ID.toString()};
        JSONArray labelSupportArray = JSONArray.parseArray(JSON.toJSONString(labelSupports));
