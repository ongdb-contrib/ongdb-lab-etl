...
//            System.out.print("\tdist数组"+toString(dist));
    ...
    public void shortestPath()                  //求带权图每对顶点间的最短路径及长度，Floyd算法
    {
        ...
        if (DEBUG) System.out.println("dist" + dist.toString() + "path" + path.toString() + "路径矩阵：");
        printPathAll(path);
        ...
        for (int k = 0; k < n; k++)                            //以vk作为其他路径的中间顶点
        {
            ...
            for (int i = 0; i < n; i++)                        //测试每对从vi到vj路径长度是否更短
                if (i != k)
                    for (int j = 0; j < n; j++)
                        if (j != k && j != i) {
                            if (DEBUG) System.out.print(toPath(path, i, j) + "路径长度" + dist.get(i, j) + "，替换为" +
                            ...
                            } else if (DEBUG) System.out.println("否");
                        }
            if (DEBUG) System.out.println("dist" + dist.toString() + "path" + path.toString() + "路径矩阵：");
            printPathAll(path);
        }
    ...
    private void printPathAll(Matrix path)                 //输出path路径矩阵中每对顶点间的路径字符串
    {
        for (int i = 0; i < path.getRows(); i++) {
            for (int j = 0; j < path.getRows(); j++)
                if (DEBUG) System.out.print(toPath(path, i, j) + " ");
            if (DEBUG) System.out.println();
        }
    }
}